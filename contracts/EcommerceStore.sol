...
contract EcommerceStore {
    ...
    /*
       opis: male je mix bosanskog i engleskog, We keep track of who inserted the product through the mapping, 
       znaci user koji hoce nesto kao da proda unese podatke proizvoda koji prodaje. The key is the merchant's account address
       ...
       an iphone to the store to sell. Our stores mapping would now have: since everyone on machinepicker has an
       accound made with classic email and password as well as account address, we will make the following: na kraju
       kada ovo bude gotovo ovaj dapp se ubacuje u machinepicker/zaposljavanje repo. na koje mjesto? profil usera.
       sta ce tu biti? upload slika koji ce biti preko ovog dappa na profilu usera imaju tri polja za upload i 3 polja 
       za unos teksta. kada neki user ode na machinepicker/zaposljavanje sire i vidi oglas, on klikne na oglas nekog
       proizvoda, to je sve klasika u mysql, onda moze kliknuti na profil ponudaca, i tu je sve klasika, podacti koji
       se ubace prilikom registracije, ALI: prilikom registracije se ubaci i account address. e sada ovde je most za
       ovaj dapp. kada odes na dno profila usera imas ono ime firme, pa sta radi, pa gdje je itd... ali user ima onda
       dole opciju da onese neke od svokih podatak u dapp, u blockchain, za ovo ti ide sema narcrtana kada ti budem
       slao opis projekta. ovaj repo ima mnogo vise coda nego sto nama treba. to cu ti osti napisati koji dio samo nam treba. 
